...
The nearest-neighborhood methods use observations in the training set $\mathcal{T}: \{(x_i, y_i),\;i=1,2,...,N\}$ closest in input space to $x$ to obtain $\hat{Y}$. We define the $k$-nearest neighbor as

$$
\hat{Y}(x) = \frac{1}{k}\sum_{x_i\in N_k(x)} y_i
$$

, where $N_k(x) = \{x_i \in \mathcal{T}:d(x_i,x) \leq d_{(k)}(x)\}$ is the $k$-nearest neighborhood of point $x$ and $|N_k| = k$. Here $d_{(k)}(x)$ denotes the $k$-th smallest distance from $x$ to the training points,  and $d(x_i,x)$ is the Euclidean norm $||x_i-x||_2$.[^6] 
...
```
We 
can further define $k$-NN more formally as:

Given test point $\textbf{x}$, we define the set of the $k$ nearest neighbors of the neighborhood $N_k$. $N_k$ is formally defined as $N_k \subseteq D$ s.t. $|N_k| = k$. Furthermore, given $\forall(\textbf{x}', y') \in D\backslash N_k$,[^7]

$$
d(\textbf{x}, \textbf{x}') \geq \argmax_{(\textbf{x}'', y'') \in N_k} d(\textbf{x}, \textbf{x}'')
$$
...
[^7]: Every point in $D$ but not in $N_k$, is at least as far away from $\textbf{x}$ as the furthest point in $N_k$


<!--chapter:end:Chp2/overview.Rmd-->
