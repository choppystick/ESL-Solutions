...
# knn implementation 
knn_model <- knn(train = train_features, test = grid_data[, c("x1", "x2")], cl = train_labels, k=15)
...
It appears that $k$-nearest neighbor fits have a single parameter, which is the number of neighbors $k$, compared to the $p$ parameters in least-squares fit. However, the effective number of parameters of $k$-nn is $N/k$ and is generally bigger than $p$, which decreases with increasing $k$. Consider the hypothetical case where the neighborhoods are non-overlapping, given $N$ training points and $k$ points per neighborhood, we will have $N/k$ neighborhoods and, a majority vote will be casted for each neighborhood, which effectively constitutes a parameter.
