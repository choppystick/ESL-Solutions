...
, where $N_k(x) = \{x_i \in \mathcal{T}:d(x_i,x) \leq d_{(k)}(x)\}$ is the $k$-nearest neighborhood of point $x$ and $|N_k| = k$. Here $d_{(k)}(x)$ denotes the $k$-th smallest distance from $x$ to the training points,  and $d(x_i,x)$ is the Euclidean norm $||x_i-x||_2$.[^6]. Similarly, the fitted values $\hat{Y}$ is converted to a class label $\hat{G}$ as in Equation \@(eq:classlabels).

```{r KNNexample, echo=FALSE, fig.align="center", fig.cap="Example of a 3-NN algorithm."}
knitr::include_graphics("C:\\Users\\samuel.chan17\\Documents\\ESL-Solutions\\Chp2\\pictures\\note2_1.png")
```
...
[^5]: Essentially, the $p$-dim predicted surface generated by the statistical model is determined by the $p$ parameters. Example: A simple linear regression of the form $y = \beta_0 + \beta_1x_1$ forms a line, characterized by ($\beta_0$, $\beta_1$) where all possible values reside. On the other hand, a linear regression of two input variables $y = \beta_0 + \beta_1x_1 + \beta_2x_2$ forms a plane, characterized by ($\beta_0$, $\beta_1$, $\beta_2$) where all possible values reside.
